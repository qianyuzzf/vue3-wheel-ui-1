<template>
  <div>
    <div class="tabs-wheel-header">
      <div class="tabs-wheel-nav1" :class="{'tabs-wheel-active':active}" @click="toggle1">{{ tabsText.title[0] }}</div>
      <div class="tabs-wheel-nav2" :class="{'tabs-wheel-active':!active}" @click="toggle2">{{ tabsText.title[1] }}</div>
      <div class="tabs-wheel-line" ref="indicator"></div>
    </div>
    <div class="tabs-wheel-main">
      <div class="tabs-wheel-content1" v-if="active">{{ tabsText.content[0] }}</div>
      <div class="tabs-wheel-content2" v-if="!active">{{ tabsText.content[1] }}</div>
    </div>
  </div>
</template>
...
export default {
  props: {
    tabsText: {
      type: Object,
      default: {title: ['导航一', '导航二'], content: ['内容一', '内容二']}
    }
  },
  setup() {
